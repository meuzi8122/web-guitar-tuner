...
import type { Tuning } from "#/domains/entities/tuner";
...
export const useTuner = (tunings: Tuning[]) => {
	...
	const updateDiff = (params: { frequency: number }) => {
		setCurrentTunings((prevTunings) => {
			return prevTunings.map((tuning) =>
				tuning.selected
					? { ...tuning, diff: FREQUENCIES[tuning.note] - params.frequency }
					: tuning,
			);
		});
	};
